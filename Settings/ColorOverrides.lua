...
function SparkPointColorOverridesMixin:OnLoad()
	...
	-- Disable default hover background; Blizzard's color override rows don't highlight.
	if self.Tooltip and self.Tooltip.HoverBackground then
		self.Tooltip.HoverBackground:SetAlpha(0)
	end
end

function SparkPointColorOverridesMixin:Init(initializer)
...
function SparkPointColorOverridesMixin:SetupRow(frame, entry)
	frame.data = entry
	if frame.Text then
...
function SparkPointColorOverridesMixin:ApplyTextColor(frame)
...
function SparkPointColorOverridesMixin:ResolveColor(key, useDefault)
	local r, g, b, a
	if useDefault then
...
function SparkPointColorOverridesMixin:ApplyColor(key, r, g, b, a)
...
function SparkPointColorOverridesMixin:RefreshRow(frame)
...
function SparkPointColorOverridesMixin:RefreshAll()
	for _, frame in ipairs(self.colorOverrideFrames or {}) do
		self:RefreshRow(frame)
	end
end

function SparkPointColorOverridesMixin:ResetToDefaults()
...
function SparkPointColorOverridesMixin:OpenColorPicker(frame)
	if not (self.setColor or self.setColorMixin) then
		return
	end
...
function SparkPointColorOverridesMixin:Release()
...
function SparkPointColorOverridesMixin:EvaluateState()
